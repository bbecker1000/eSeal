...
## get from SealTrend Script before transposed
dat3 <- t(eSeal)
###################################################
...
dat <- log(dat3)     ## log the data
dat3[dat3 == 0] <- NA## replace zeros with NA
dat3[dat3 == -Inf] <- NA  ##QC why we still have some -Inf
